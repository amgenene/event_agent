...
class CalendarService:
    ...
    def get_availability(self, participants: list[str]) -> list:
        ...
        end_of_week = (now + timedelta(days=days_until_sunday)).replace(
            ...
            minute=55,
            second=0,
            microsecond=0,  # 23:55 to ensure multiple of 5
        )
        ...
        logger.debug(
            "Nylas API Request: participants=%s start=%s end=%s duration=60 interval=30",
            [p["email"] for p in participants_array],
            datetime.fromtimestamp(start_time, tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M %Z"
            ),
            datetime.fromtimestamp(end_time, tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M %Z"
            ),
        )
        ...
        try:
            ...
            elif hasattr(availability_response, "data") and hasattr(
                availability_response.data, "time_slots"
            ):
            ...
            logger.exception("Nylas API Error: %s", e)
            raise
